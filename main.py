...
class TrafficMonitor(simple_switch_13.SimpleSwitch13):
    ...
    def reset(self):
        ...
        #     self.add_meter_band(self.datapaths[i], MAX_BANDWIDTH)
            self.add_meter_band(self.datapaths[key], MAX_BANDWIDTH)
            pass
        self.get_state()
    ...
    def main(self):
        ...
        # placeholders
        state_placeholder = tf.compat.v1.placeholder(
            dtype=tf.float32, shape=[None, STATE_DIM])
        action_placeholder = tf.compat.v1.placeholder(
            dtype=tf.float32, shape=[None, ACTION_DIM])
        reward_placeholder = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None])
        next_state_placeholder = tf.compat.v1.placeholder(
            dtype=tf.float32, shape=[None, STATE_DIM])
        # indicators (go into target computation)
        is_not_terminal_placeholder = tf.compat.v1.placeholder(
        ...
        with tf.compat.v1.variable_scope('target_actor', reuse=False):
        ...
        with tf.compat.v1.variable_scope('target_critic', reuse=False):
        ...
        actor_vars = tf.compat.v1.get_collection(
            tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope='actor')
        target_actor_vars = tf.compat.v1.get_collection(
            tf.compat.v1.GraphKeys.GLOBAL_VARIABLES, scope='target_actor')
        critic_vars = tf.compat.v1.get_collection(
            tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope='critic')
        target_critic_vars = tf.compat.v1.get_collection(
            tf.compat.v1.GraphKeys.GLOBAL_VARIABLES, scope='target_critic')
        ...
        # critic optimizer
        critic_train_op = tf.compat.v1.train.AdamOptimizer(
        ...
        actor_train_op = tf.compat.v1.train.AdamOptimizer(
